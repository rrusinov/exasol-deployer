...
Options:
  ...
  --provider <NAME>  Only check specific provider (aws, azure, gcp, oci, hetzner, digitalocean, exoscale, libvirt)
...
collect_oci_data_parallel() {
    ...
    if [[ -z "$compartment_ocid" ]]; then
        echo "Could not determine OCI compartment OCID" >&2
        return 1
    fi
    ...
    generate_oci_html_report "$compartment_ocid" "$region" "$temp_dir/oci-$region.html"
}
...
collect_hetzner_data() {
    ...
    generate_hetzner_html_report "$output_file"
}
...
generate_html() {
    ...
    local providers=("aws" "azure" "gcp" "oci" "hetzner" "digitalocean" "exoscale" "libvirt")
    ...
    for prov in "${providers[@]}"; do
        case "$prov" in
            ...
            oci)
                ...
                ;;
            hetzner)
                collect_hetzner_data "$temp_dir/hetzner.html" &
                ;;
            digitalocean)
                collect_digitalocean_data "$temp_dir/digitalocean.html" &
                ;;
            exoscale)
                collect_exoscale_data "$temp_dir/exoscale.html" &
                ;;
            libvirt)
                collect_libvirt_data "$temp_dir/libvirt.html" &
                ;;
        esac
    done
    wait
    ...
    for prov in "${providers[@]}"; do
        ...
        case "$prov" in
            ...
            oci)
                ...
                region=$(oci iam region-subscription list --query "data[?\"is-home-region\"].\"region-name\" | [0]" --raw-output 2>/dev/null || echo "unknown")
                if [[ -f "$temp_dir/oci-$region.html" ]]; then
                    cat "$temp_dir/oci-$region.html" >> "$output_file"
                fi
                ;;
            hetzner)
                if [[ -f "$temp_dir/hetzner.html" ]]; then
                    cat "$temp_dir/hetzner.html" >> "$output_file"
                fi
                ;;
            digitalocean)
                if [[ -f "$temp_dir/digitalocean.html" ]]; then
                    cat "$temp_dir/digitalocean.html" >> "$output_file"
                fi
                ;;
            exoscale)
                if [[ -f "$temp_dir/exoscale.html" ]]; then
                    cat "$temp_dir/exoscale.html" >> "$output_file"
                fi
                ;;
            libvirt)
                if [[ -f "$temp_dir/libvirt.html" ]]; then
                    cat "$temp_dir/libvirt.html" >> "$output_file"
                fi
                ;;
        esac

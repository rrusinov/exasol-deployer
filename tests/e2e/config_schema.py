...
def validate_workflow_step(step: Dict, provider: str) -> List[str]:
    ...
    if schema.supported_providers and provider not in schema.supported_providers:
        ...
        errors.append(
            f"Supported providers: {', '.join(sorted(schema.supported_providers))}"
        )
    ...
    if step_type == "validate":
        ...
        if has_command and has_script:
            errors.append(
                "Step 'custom_command' cannot specify both 'command' and 'script'"
            )
...
def validate_validation_check(check: str) -> List[str]:
    ...
    if check.startswith("cluster_status"):
        if "==" in check or "!=" in check:
            ...
            if value not in VALID_CLUSTER_STATUSES:
                ...
                errors.append(
                    f"Valid values: {', '.join(sorted(VALID_CLUSTER_STATUSES))}"
                )
        else:
            errors.append(f"Invalid cluster_status check format: {check}")
            errors.append(
                f"Expected format: {VALIDATION_CHECK_PATTERNS['cluster_status']}"
            )
    ...
    # Check health_status pattern
        ...
        if not match:
            ...
            errors.append(
                f"Expected format: {VALIDATION_CHECK_PATTERNS['health_status']}"
            )
        else:
            ...
            if component not in HEALTH_CHECK_COMPONENTS:
                ...
                errors.append(
                    f"Valid components: {', '.join(sorted(HEALTH_CHECK_COMPONENTS.keys()))}"
                )

            if value.lower() not in VALID_HEALTH_VALUES:
                errors.append(f"Unknown health check value: {value}")
                errors.append(
                    f"Valid values: {', '.join(sorted(VALID_HEALTH_VALUES.keys()))}"
                )
    else:
        errors.append(f"Unknown validation check format: {check}")
        errors.append(
            f"Supported patterns: {', '.join(VALIDATION_CHECK_PATTERNS.keys())}"
        )
...
def validate_sut_parameters(parameters: Dict, provider: str) -> List[str]:
    ...
    for param_name, param_value in parameters.items():
        ...
        # Check provider support
        if (
            param_schema["providers"] != "all"
            and provider not in param_schema["providers"]
        ):
            errors.append(
                f"Parameter '{param_name}' not supported for provider: {provider}"
            )
            errors.append(
                f"Supported providers: {', '.join(sorted(param_schema['providers']))}"
            )
